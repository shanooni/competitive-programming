...
Valera reads each book up to the end, that is, he doesn't start reading the book if he doesn't have enough free time to
finish reading it.
...
def booksReadInTime(n: int, time: int, books: list[int]) -> int:
    books.sort()
    timeLeft = 0
    booksRead = 0
    for i in range(n):
        while booksRead <= len(books):
            timeLeft = time - books[i]
            booksRead += 1
    return booksRead


print(booksReadInTime(4, 5, [3, 1, 2, 1]))
